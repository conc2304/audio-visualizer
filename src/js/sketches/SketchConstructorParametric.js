...
class Parametric {
  constructor(windowWidth, windowHeight, p5) {
    ...
    this.width = new NumericProperty('Width', 'Base', 400, 50, 750, 0.7);
    this.height = new NumericProperty('Height', 'Base', 400, 50, 750, 0.7);
...
Parametric.prototype.render = function(p5) {
  ...
  this.x = 0;
  this.y = 0;
  // shape.q += this.velocity.currentValue;
  ...
  for (let i = 0; i < this.iter.currentValue; i++) {
    ...
    // const position = this.parametricRing(p5);
    const position = this.parametric006(p5);
...
Parametric.prototype.parametricFullScreenPattern002 = function(p5) {
  ...
  let m = map(p5.sin(this.q), 0.000001, 1000, 10, 500);
...
Parametric.prototype.parametricTeardrop = function(p5) {
  return {
    x: (p5.cos(this.q + p5.sin(this.theta)) * this.windowWidth) / 3 + this.windowWidth / 2 + this.translateX,
    y: (p5.cos(this.theta + this.q) * windowHeight) / this.height + this.windowHeight / 2 + this.translateY,
  };
}
...
Parametric.prototype.parametric006 = function(p5) {
  return {
    x:
      (p5.sin(shape.theta + p5.sin(shape.theta + shape.q))),
    y:
      (p5.cos(shape.theta + p5.sin(shape.theta - shape.q))),
  };
}
Parametric.prototype.parametric009 = function(p5) {
  return {
    x:
      ...
      this.windowWidth / 2 +
      this.translateX,
    y: (p5.cos(this.theta) * this.windowHeight) / this.height + this.windowHeight / 2 + this.translateY,
  };
}
